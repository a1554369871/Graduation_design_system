  ...
  <div>
    ...
    <template v-else>
      <el-card shadow="hover" style="margin-bottom:16px">
        <template #header><span>最终稿提交</span></template>
      ...
      <el-card v-if="stageStatus === 'locked'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="最终稿提交阶段未开启，请先完成查重定稿。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'approved'" shadow="hover">
        <el-alert type="success" show-icon :closable="false" title="最终稿已通过审核，项目已完成！" />
      </el-card>
      <el-card v-else shadow="hover" style="margin-bottom:16px">
        <template #header><span>提交最终材料</span></template>
        <el-alert type="info" show-icon :closable="false" title="请在此提交查重定稿后的最终材料（含最终版论文、查重报告等）。" style="margin-bottom:16px" />
        <el-form label-width="80px" style="max-width:500px" @submit.prevent>
          ...
          <el-form-item>
            <el-button type="success" :loading="submitting" @click="handleSubmit" :disabled="!selectedFile">提交最终材料</el-button>
          </el-form-item>
        </el-form>
      </el-card>
        ...
        <el-timeline>
          <el-timeline-item v-for="sub in submissions" :key="sub.id" :timestamp="sub.created_at" placement="top">
            <el-card shadow="hover" size="small">
              <div>
                <el-tag size="small" type="success" style="margin-right:8px">最终稿</el-tag>
                <span style="margin-right:4px">v{{ sub.version }}</span>
                {{ sub.file_name }}
