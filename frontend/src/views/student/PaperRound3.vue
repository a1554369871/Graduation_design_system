  ...
  <div>
    ...
    <template v-else>
      <el-card shadow="hover" style="margin-bottom:16px">
        <template #header><span>三轮修改</span></template>
      ...
      <el-card v-if="stageStatus === 'locked'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="三轮修改阶段未开启，请先完成二轮修改。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'approved'" shadow="hover">
        <el-alert type="success" show-icon :closable="false" title="三轮修改已通过评审，即将进入查重定稿阶段。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'completed'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="三轮修改阶段已完成，请进入下一阶段。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'rejected'" shadow="hover">
        <el-alert type="error" show-icon :closable="false" title="三轮修改未通过，无法继续提交，请联系管理员。" />
      </el-card>
      <el-card v-else shadow="hover" style="margin-bottom:16px">
        <template #header><span>提交三轮修改稿</span></template>
        <el-alert type="warning" show-icon :closable="false" title="这是最后一次修改机会，请认真修改。" style="margin-bottom:16px" />
        <el-form label-width="80px" style="max-width:500px" @submit.prevent>
          ...
          <el-form-item>
            <el-button type="primary" :loading="submitting" @click="handleSubmit" :disabled="!selectedFile">提交三轮修改稿</el-button>
          </el-form-item>
        </el-form>
      </el-card>
...
function getStageStatus() {
  ...
  if (subs.length > 0) {
    ...
    if (last.reviews?.some(r => !r.is_approved)) return 'rejected'
  }
  return 'current'
}
