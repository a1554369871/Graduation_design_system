  ...
  <div>
    ...
    <template v-else>
      <el-card shadow="hover" style="margin-bottom:16px">
        <template #header><span>一轮修改</span></template>
      ...
      <el-card v-if="stageStatus === 'locked'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="一轮修改阶段未开启，请先完成初稿评审。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'approved'" shadow="hover">
        <el-alert type="success" show-icon :closable="false" title="一轮修改已通过评审，即将进入查重定稿阶段。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'completed'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="一轮修改阶段已完成，请进入下一阶段。" />
      </el-card>
      <el-card v-else shadow="hover" style="margin-bottom:16px">
        <template #header><span>提交一轮修改稿</span></template>
        <el-form label-width="80px" style="max-width:500px" @submit.prevent>
          ...
          <el-form-item>
            <el-button type="primary" :loading="submitting" @click="handleSubmit" :disabled="!selectedFile">提交一轮修改稿</el-button>
          </el-form-item>
        </el-form>
      </el-card>
