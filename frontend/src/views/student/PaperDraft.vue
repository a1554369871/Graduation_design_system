  ...
  <div>
    ...
    <template v-else>
      <el-card shadow="hover" style="margin-bottom:16px">
        <template #header><span>初稿提交</span></template>
      ...
      <el-card v-if="stageStatus === 'locked'" shadow="hover">
        <el-alert type="info" show-icon :closable="false" title="初稿阶段未开启，请先完成选题审核。" />
      </el-card>
      <el-card v-else-if="stageStatus === 'approved'" shadow="hover">
        <el-alert type="success" show-icon :closable="false" title="初稿已通过评审，请进入一轮修改阶段继续提交。" />
      </el-card>
      <el-card v-else shadow="hover" style="margin-bottom:16px">
        <template #header><span>提交初稿</span></template>
        <el-form label-width="80px" style="max-width:500px" @submit.prevent>
          <el-form-item label="说明"><el-input v-model="description" placeholder="可选的提交说明" /></el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" :loading="submitting" @click="handleSubmit" :disabled="!selectedFile">提交初稿</el-button>
          </el-form-item>
        </el-form>
      </el-card>
...
function getStageStatus() {
  ...
  const sortOrder = {
    topic_selection: 1, first_draft: 2, round1: 3, round2: 4,
    round3: 5, final_check: 6, final_submission: 7, defense: 8, archived: 9,
  }
  const cs = sortOrder[code] || 0
  if (cs > 2) return 'completed'
  if (cs < 2) return 'locked'
  const draftSubs = submissions.value.filter(s => s.submission_type === 'draft')
  if (draftSubs.length > 0) {
    const lastDraft = draftSubs[draftSubs.length - 1]
    if (lastDraft.reviews?.some(r => r.is_approved)) return 'approved'
  }
  return 'current'
}
...
async function fetchData() {
  try {
    ...
    project.value = res
    submissions.value = (res.submissions || []).filter(s => s.submission_type === 'draft')
